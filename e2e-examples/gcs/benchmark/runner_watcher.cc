...
void RunnerWatcher::NotifyCompleted(
    OperationType operationType, int32_t runner_id, int64_t channel_id,
    std::string peer, std::optional<bool> on_directpath, std::string bucket,
    std::string object, grpc::Status status, int64_t bytes, absl::Time time,
  ...
  op.peer = peer;
  op.on_directpath = on_directpath;
  op.bucket = bucket;
  op.object = object;
  op.status = status;
  op.bytes = bytes;
  ...
  if (verbose_) {
    ...
    printf(
        "### %sCompleted: ord=%ld time=%s peer=%s dp=%s bucket=%s object=%s "
        ...
        peer.c_str(),
        on_directpath.has_value() ? (on_directpath.value() ? "O" : "X") : "?",
        bucket.c_str(), object.c_str(), (long long)bytes, sec,
